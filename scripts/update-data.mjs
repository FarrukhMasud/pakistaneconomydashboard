...
// Monthly direct investment in Pakistan (BOP BPM6)
const FDI_MONTHLY_SERIES = {
  net_fdi: { key: 'TS_GP_BOP_BPM6SUM_M.P00330', name: 'Net Direct Investment in Pakistan' },
  equity: { key: 'TS_GP_BOP_BPM6SUM_M.P00340', name: 'Equity and Reinvested Earnings' },
  debt: { key: 'TS_GP_BOP_BPM6SUM_M.P00350', name: 'Debt Instruments' },
};
...
async function updateFdiMonthly(apiKey) {
  console.log('\n💰 Updating monthly FDI from SBP EasyData API...');

  try {
    const existing = await readJson('fdi.json');
    const seriesData = {};
    ...
    for (const [field, s] of Object.entries(FDI_MONTHLY_SERIES)) {
      ...
      seriesData[field] = rows
        ...
        .sort((a, b) => a.date.localeCompare(b.date));
      console.log(`    ${s.name}: ${seriesData[field].length} monthly observations`);
    }

    const byDate = new Map();
    for (const [field, rows] of Object.entries(seriesData)) {
      for (const row of rows) {
        if (!byDate.has(row.date)) byDate.set(row.date, { date: row.date });
        byDate.get(row.date)[field] = row.value;
      }
    }
    ...
    console.error(`  ❌ ${err.message}`);
    logResult('FDI Monthly', 'failed', err.message);
  }
}
